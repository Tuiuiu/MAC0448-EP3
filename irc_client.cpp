...
void IRC_Client::process_command(std::string command, Host & host)
{
    ...
    { // precisa de DNS
    	...
    	host.datagram_queue.emplace_back(host.ip_, host.dns_server_ip_, new UDP_Segment(DNS_CLIENT_PORT, DNS_SERVER_PORT, content));
    	...
    	|| std::regex_search(command, result, std::regex("^QUIT$")))
    {
    	...
    	host.datagram_queue.emplace_back(source_ip, destination_ip, new UDP_Segment(source_port, destination_port, command));
    }
    else
